...
(function(c) {
	...
	var a = {
		onDialogOpen : function() {
			var d = this.settings;
			this.queueData.errorMsg = "文件添加失败";
		...
		onUploadProgress : function(g, m, j) {
			...
			if (c.inArray("onUploadProgress", h.overrideEvents) < 0) {
				if (h.progressData == "percentage") {
					c("#" + g.id).find(".data").html(" " + p + "%"+",速度"+this.queueData.averageSpeed + o+",已上传"+getMegaByte(d));
				} else {
