...
KindEditor.lang({
	...
	'multiimage.uploadDesc' : '可同时上传<%=uploadLimit%>张图片，单张图片容量不超过<%=sizeLimit%>(目前只支持.jpg .jpeg)',
...
KindEditor.plugin('insertfile', function(K) {
	...
	self.plugin.fileDialog = function(options) {
		...
		var html = [
			...
			'<input type="button" id="_trigger_upload" class="ke-upload-button" value="' + lang.upload + '" /> &nbsp;',
			'<span class="ke-button-common ke-button-outer">',
			'<input type="button"  class="ke-button-common ke-button" name="viewServer" value="' + lang.viewServer + '" />',
			'</span>',
			'</div>',
			...
			'<input type="text" id="keTitle" class="ke-input-text" name="title" value="" style="width:80px;" /></div>',
			
			'<div class="ke-dialog-row">',
			'<ul id="_file_quene" style="height:20px;"></ul></div>',
			'</div>',
			//form end
			'</form>',
			'</div>'
		...
		if (allowFileUpload) {
			//加载上传组件，完成后绑定上传组件
			In.ready("upload",function(){
				//_trigger_upload
				var _kindeditor_uploader = new plupload.Uploader({
					 runtimes : 'flash',
		    		 browse_button : '_trigger_upload', 
		    		 max_file_size : '200mb',
		    		 url : 'http://oss.haoteacher.com.cn/upload/create?appid=8500&uid=6724&refid=90&reftype=0&transcoding=0',
		    		 flash_swf_url : '/dt_static/js/plugins/plupload/Moxie.swf',
		    		 multi_selection:false //不允许多选
				});
				
				_kindeditor_uploader.init();
				_kindeditor_uploader.bind('FilesAdded',function(up, files){
					 $.each(files, function(i, file) {
						 $('#_file_quene').html("<li>"+file.name+"("+plupload.formatSize(file.size)+")<b id='"+file.id+"_current'>0%</b></li>");
					 });
					 _kindeditor_uploader.start();
					 //dialog.showLoading(self.lang('uploadLoading'));
				});
				//调整进度
				_kindeditor_uploader.bind('UploadProgress',function(up, file){
					 $('#' + file.id + "_current").html(file.percent+"%");
				});
				//上传错误
				_kindeditor_uploader.bind('Error',function(up, err){
					 alert("\nError #"+err.code + ": " + err.message)
				});
				//文件上传完成
				_kindeditor_uploader.bind('FileUploaded', function(up,file,info) {
					 $('#' + file.id + "_current").html("100%");
					 var one = JSON.parse(info.response);
					 if(one.code==1){ //上传成功
						 var url = one.url ;
						 if(formatUploadUrl){
							 url = K.formatUrl(url, 'absolute');
							 urlBox.val(url);
							 if(self.afterUpload){
								 self.afterUpload.call(self, url, one, name);
							 }
						 }
					 }else{
						 alert('文件保存失败');
...
KindEditor.plugin('multiimage', function(K) {
	...
	self.plugin.multiImageDialog = function(options) {
		...
		var swfupload = K.swfupload({
			...
			flashUrl : 'http://2013shifan.yx.teacher.com.cn/dt_static/js/plugins/kindeditor/plugins/multiimage/images/swfupload.swf',
